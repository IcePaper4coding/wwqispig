...
<div :class="'editorbox '+'editorbox'+this.keys" :id="'editorbox'+keys">
    ...
    <el-input v-model="titleinput" :disabled="mode=='VIEW'" :placeholder="mode=='VIEW'?'':'请输入标题'" :style="needTitle?'':'display:none'" class="titleboxitem" @keydown.native.enter="istoedit"></el-input>
    <input type="file" @change="upload" class="uploadstyle" id="uploadstyle">
    
    <div class="pageDtl_navbox" :style="mode=='VIEW'?'top:105px;right: 0px;':''" @mouseenter="genCatalog" v-if="needCatalog">
    <div class="linebpx">
    </div>
...
export default {
    ...
    mounted() {
        this.init()
        document.getElementsByClassName('ql-header')[0].addEventListener('click',()=>{
            if((this.thiseditor.getSelection())){
                ...
                var num = this.thiseditor.getBounds((this.thiseditor.getSelection()).index).top
                this.$nextTick(()=>{
                    if(document.getElementById(Math.floor(num))){
                        document.getElementById(Math.floor(num)).scrollIntoView({
                            block:"center"
                        })
                    }
                    else if(document.getElementById(Math.floor(num)+1)){
                        document.getElementById(Math.floor(num)+1).scrollIntoView({
                            block:"center"
                        })
                    }
                    else if(document.getElementById(Math.floor(num)-1)){
