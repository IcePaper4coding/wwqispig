<template>
    <div id="yb-editor" :style="'width:' + propObj.widthStr + ';height:' + (propObj.heightStr || fullHeightStr)">
        <div class="yb-editor__toolbar-box" :style="'width:' + (propObj.widthStr || fullWidthStr)" v-if="!propObj.disabledBool">
...
<script>
    ...
    /* 引入表格模块 */
    import QuillCustomTable from 'quill-better-table'
    import 'quill-better-table/dist/quill-better-table.css'
    // import 'npmpaas_icon'
    Quill.register({
        'modules/better-table': QuillCustomTable
    }, true);
            ...
            initQuillFunc() {
                ...
                this.quillObj = new Quill('#' + (this.propObj.keyStr || 'yb-editor__body'), {
                    ...
                    modules: {
                        syntax: false,/* 语法高亮 */
                        toolbar: {
                            container: this.propObj.disabledBool ?'#nothingTool':'#yb-toolbar_' + this.propObj.keyStr,
                            handlers: {
                                    ...
                                    if (fileInput == null) {
                                        ...
                                        fileInput.addEventListener('change', ()=> {
                                            ...
                                            if (fileInput.files != null && fileInput.files[0] != null) {
                                                ...
                                                vm.$http.upload(formObj,file.name).then(res=>{
                    ...
                    theme: 'snow' /* 主体色 snow bubble*/,
                    placeholder: "" /* 提示文字 */,
                    readOnly: this.propObj.disabledBool ? true : false /* 是否只读 */,
                });
                this.tableMoudle = this.quillObj.getModule('better-table');
            },
            /* 显示设置表格行列的选择器 */
            showPickerFn(){
